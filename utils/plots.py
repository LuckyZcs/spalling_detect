...
def plot_one_box(x, im, color=(128, 128, 128), label=None, line_thickness=3, save_path=''):
    ...
    # cv2.imwrite('./results/{}.jpg'.format(save_path.split('.')[0].split('\\')[-1]),
    #             im[int(x[1]):int(x[3]), (center_x-1800):(center_x+1800)])
    # cv2.imwrite('./results/{}.jpg'.format(save_path.split('.')[0].split('\\')[-1]),
    #             im[int(x[1]):int(x[3]), int(x[0]):int(x[2])])
